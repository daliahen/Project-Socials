...
public class DbUtils { // כל השאילתות יעברו לפה , צריך להגיד איך מתחברים לDATABASE
    ...
    public List<User> getAllUsers() {
        try {
            ...
            PreparedStatement preparedStatement = this.connection.prepareStatement
                    ("SELECT id , username , password , image_url FROM users");
    ...
    public User getUserByUsername(String username ) {
        try {
            PreparedStatement preparedStatement = this.connection
                    .prepareStatement( "SELECT id, username, password, image_url FROM users WHERE username = ?");
            preparedStatement.setString(1,username);
    ...
    public void createPostOnDb(long authorUserId , String text) {
        try {
            PreparedStatement preparedStatement = this.connection
                    .prepareStatement("INSERT INTO posts(author_user_id, text)" +
                    "VALUES (? , ?)");
    ...
    public List<Post> getAllPosts(long authorUserId) {
        ...
        try {
            ...
            PreparedStatement preparedStatement =  this.connection
                    .prepareStatement("SELECT id , author_user_id , text , created_at FROM posts" +
